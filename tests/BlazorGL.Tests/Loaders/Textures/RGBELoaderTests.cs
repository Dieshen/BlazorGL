...
public class RGBELoaderTests
{
    ...
    public async Task LoadAsync_DecodesRGBECorrectly()
    {
        ...
        // Raw decoded value: (255, 128, 64) * 2^(128-128) / 256 = (0.99609375, 0.5, 0.25)
        // After gamma correction (gamma=2.2): value^(1/2.2)
        var rgbeData = CreateRGBEFileWithPixel(255, 128, 64, 128);
        var loader = CreateLoader(rgbeData);
        loader.Gamma = 1.0f; // Disable gamma correction for predictable test values
